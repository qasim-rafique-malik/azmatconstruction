...
if (class_exists('Woocommerce')) {
	...
	function new_loop_shop_per_page( $cols ) {
		$num = buildpro_get_option('per_shop') ? buildpro_get_option('per_shop') : 6;
	...
	add_filter( 'woocommerce_output_related_products_args', 'buildpro_related_products_args' );
	function buildpro_related_products_args( $args ) {
	...
	add_filter( 'woocommerce_add_to_cart_fragments', 'buildpro_header_add_to_cart_fragment' );
	function buildpro_header_add_to_cart_fragment( $fragments ) {
		ob_start();
